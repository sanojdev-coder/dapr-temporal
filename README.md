...
4. If no signal arrives within 10s, the workflow fails safe: `BLOCKED` /
   `FRAUD_SERVICE_TIMEOUT`, still published so the audit trail isn't lost.
...
- **Temporal**: added `PROMETHEUS_ENDPOINT: 0.0.0.0:9090` to the `temporal`
...
(`temporal_workflow_completed_total`, `dapr_http_server_response_latency_seconds_bucket`,
...
- `PaymentResult` unifies `processed_at`/`blocked_at` from the original spec
...
1. `DYNAMIC_CONFIG_FILE_PATH` was pointed at a file that doesn't exist in
   ...
   set it.
2. `depends_on` only waits for a container to *start*, not for the service
   ...
   blocks on a raw TCP connection to `TEMPORAL_SERVICE_ADDRESS` before
   `exec`-ing the JVM.
